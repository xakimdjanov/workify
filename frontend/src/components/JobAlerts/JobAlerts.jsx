...
import { 
  ...
  FaUserTie, FaBuilding, FaHome, FaFlag, FaSearch, 
  FaMapMarkerAlt, FaTrashAlt, FaEye 
} from "react-icons/fa";
...
export default function JobAlerts() {
  ...
  const fetchApplications = async () => {
    try {
      ...
      // Backend ma'lumotlari massiv ichida kelayotganini tekshiramiz
  ...
  // 2. Arizani o'chirish (Bekor qilish)
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] p-4 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        {/* --- FILTER SECTION (Rasm dizayni asosida) --- */}
        ...
        <div className="space-y-6">
          ...
          {loading ? (
            <div className="text-center py-20 animate-pulse text-gray-400 font-bold">Loading applications...</div>
              ...
              <div key={app.id} className="bg-white rounded-[32px] p-8 border border-gray-50 shadow-sm hover:shadow-md transition-all">
                <div className="flex flex-col md:flex-row justify-between gap-6">
                  ...
                  <div className="md:text-right">
                    <p className="text-2xl font-black text-slate-800">${app.job?.salary_max}.00</p>
                    <p className="text-gray-400 text-xs font-bold mt-1">Applied on: {new Date(app.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
