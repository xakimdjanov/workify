...
import {
  ...
  FaBuilding, FaHome, FaFlag, FaSearch, FaChevronDown,
  FaCity, FaDollarSign, FaThumbsDown
} from "react-icons/fa";
...
export default function JobMatches() {
  ...
  // Filter states
  const [employmentType, setEmploymentType] = useState(""); // Default bo'sh bo'lsa barchasini ko'rsatadi
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        // 1. User skillarini olish va tozalash
        ...
        // 2. Barcha joblarni olish
        const jobRes = await jobApi.getAll();
        const jobs = jobRes.data || [];

        // 3. Skillarga moslik bo'yicha saralash (lekin barchasini saqlash)
        const sortedJobs = [...jobs].sort((a, b) => {
          ...
          return bMatch - aMatch; // Mos keladiganlar tepaga chiqadi, qolganlar pastda qoladi
        });
  ...
  // Filtrlash mantiqi (Barcha ishlarni o'z ichiga oladi)
  const filteredJobs = allJobs.filter(job => {
    ...
    const matchSearch = searchQuery ? 
      (job.occupation?.toLowerCase().includes(searchQuery.toLowerCase()) || 
       job.company?.company_name?.toLowerCase().includes(searchQuery.toLowerCase())) : true;
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 mb-6 overflow-hidden">
          <div className="p-6 border-b border-gray-50 flex justify-between items-center">
            <h2 className="text-xl font-bold text-[#163D5C]">Job Alerts</h2>
            <button 
          ...
          <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isExpanded ? "max-h-[1000px] opacity-100" : "max-h-0 opacity-0"}`}>
            <div className="p-8 space-y-8">
              {/* Employment Type */}
              <div>
                <label className="block text-gray-700 font-bold mb-4">Employment type</label>
                  ...
                  {["Full time", "Part time", "Contract", "Freelance"].map(type => (
                    <button
                      ...
                      className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all ${
                        employmentType === type ? "bg-white text-[#163D5C] shadow-sm" : "text-gray-400 hover:text-gray-600"
                      }`}
                    >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* Workplace Type */}
                <div>
                  <label className="block text-gray-700 font-bold mb-4">Workplace type</label>
                  <div className="flex bg-[#F1F3F6] rounded-2xl p-1 gap-1 w-fit">
                    {["Onsite", "Remote", "Hybrid"].map(type => (
                      <button
                        ...
                        className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all ${
                          workplaceType === type ? "bg-white text-[#163D5C] shadow-sm" : "text-gray-400 hover:text-gray-600"
                        }`}
                      >
                ...
                {/* Minimum Salary */}
                <div>
                  <label className="block text-gray-700 font-bold mb-4">Minimum salary <span className="text-gray-300 font-medium">(USD)</span></label>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center px-2">
            <p className="text-gray-500 font-bold">{filteredJobs.length} jobs available</p>
          </div>
          ...
          {loading ? (
            ...
            filteredJobs.map((job) => {
              // Har bir job uchun moslikni tekshirish
              ...
              return (
                <div key={job.id} className={`bg-white rounded-[32px] p-8 border transition-all duration-300 ${isMatch ? "border-blue-100 bg-blue-50/10 shadow-md" : "border-gray-50 shadow-sm"}`}>
                    ...
                    <div className="flex gap-5">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <span className="text-gray-400 text-xs font-bold ml-1">(4.0) 1K reviews</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap justify-between items-center mt-6 mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      {isMatch && (
                        <span className="bg-[#163D5C] text-white text-[10px] px-3 py-1 rounded-full font-black uppercase">Best Match</span>
                      )}
                    </div>
                  ...
                  <p className="text-gray-500 text-sm font-medium leading-relaxed mb-6 line-clamp-2">
                    {job.description || "Looking for a talented professional to join our fast-growing team. You will be responsible for high-quality deliverables..."}
